...
#define XXL_IMG_W   (XXL_SIZE_W/7)
#define XXL_IMG_H   (XXL_SIZE_H/7)
...
static game_obj_type game_obj[7][7] = {
    0,
};
static int score;
...
void xiaoxiaole(void)
{
    ...
    lv_obj_align(refs_btn, LV_ALIGN_TOP_RIGHT, 18, -10);
    ...
    lv_obj_align_to(coin, refs_btn, LV_ALIGN_OUT_BOTTOM_MID, 0, -10);

    score = 0;

    score_lable = lv_label_create(screen1);
    lv_label_set_text_fmt(score_lable, "%d", score);
    ...
    for (j = 0; j < 7; j++) {
        for (i = 0; i < 7; i++) {
            if (game_obj[j][i].alive == 0 && lv_obj_is_valid(game_obj[j][i].obj)) {
                ...
                lv_anim_set_values(&a1, (i) * XXL_IMG_W * screen_ratio + 1 + lv_obj_get_x(game_window), lv_obj_get_x(coin));
                lv_anim_start(&a1);
                ...
                lv_anim_set_values(&a2, (j) * XXL_IMG_W * screen_ratio + 1 + lv_obj_get_y(game_window), lv_obj_get_y(coin));
                lv_anim_start(&a2);
            }
        }
    }
...
static void check_after_move(lv_timer_t *timer);
...
static void obj_move_down()
{
    ...
    // 如果有动画，使用定时器延迟检查；否则立即检查
    if (has_anim) {
        lv_timer_t *timer = lv_timer_create(check_after_move, 200, NULL);
        lv_timer_set_repeat_count(timer, 1);
    } else {
        check_after_move(NULL);
    }
}
...
static void move_to_coin_end_cb(lv_anim_t *a)
{
    ...
    if (lv_anim_count_running() == 0 && !processing) {
        processing = true;
        deleted_count = 0;
        lv_label_set_text_fmt(score_lable, "%d", score);
        obj_move_down();
