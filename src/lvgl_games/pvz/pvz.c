...
#define fast_zb_period           1000 
#define super_fast_zb_period     500  
#define zidan_speed              7   
#define shine_period             7000 
#define plant_stackable          0    
#define sunflower_add_sun_period 7000 
#define add_zidan_period         1000 
...
// Adaptation for 320x240
#define PVZ_SCREEN_W 320
#define PVZ_SCREEN_H 240
...
#define ZB_SPAWN_X 320
#define ZB_Y_OFFSET 23
#define ZB_Y_STEP 36

#define CARD_START_X 35
#define CARD_STEP_X 22
#define CARD_Y 2

#define EXIT_BTN_X 260
#define CHANZI_BTN_X 230

#define CAR_START_Y 43
#define CAR_STEP_Y 36

#define PLANT_X_STEP 33
#define PLANT_Y_STEP 36
#define PLANT_X_OFFSET 5
#define PLANT_Y_OFFSET 20
...
static void game_start(lv_event_t *e)
{
    ...
    lv_obj_set_pos(score_lable, 5, 20);

    for (i = 0; i < sizeof(card_btn) / sizeof(card_btn_type); i++) {
        card_btn[i].card_btn = lv_img_create(map1);
        lv_obj_set_pos(card_btn[i].card_btn, CARD_START_X + (i) * CARD_STEP_X, CARD_Y);
    ...
    lv_obj_set_pos(botton_exit, EXIT_BTN_X, 0);
    ...
    lv_obj_set_pos(chanzi_btn, CHANZI_BTN_X, -1);
    ...
    for (i = 0; i < 5; i++) {
        ...
        lv_img_set_zoom(little_car[i].zidan, 170);
        lv_obj_set_pos(little_car[i].zidan, little_car[i].x, i * CAR_STEP_Y + CAR_START_Y);
...
static void cherry_delete_cb(lv_anim_t *a)
{
    ...
    for (i = 0; i < max_zb_count; i++) {
        if (zb_matrix[i].blood > 0 && lv_obj_get_x(zb_matrix[i].zb) - x < 53 &&
            x - lv_obj_get_x(zb_matrix[i].zb) < 53 && lv_obj_get_y(zb_matrix[i].zb) - y < 53 &&
            y - lv_obj_get_y(zb_matrix[i].zb) < 53) {
            zb_matrix[i].blood = 0;
            zb_matrix[i].zb_class->zb_del_cb(&zb_matrix[i]);
        }
    }
...
static void lajiao_delete_cb(lv_anim_t *a)
{
    ...
    int y = (lv_obj_get_y(lajiao) - PLANT_Y_OFFSET - 9) / PLANT_Y_STEP;

    lv_obj_t *huoyan = lv_img_create(map1);
    lv_img_set_src(huoyan, &huoyan_img);
    lv_obj_set_y(huoyan, y * PLANT_Y_STEP + PLANT_Y_OFFSET + 25);
    lv_obj_set_size(huoyan, PVZ_SCREEN_W, 30);
...
static void plant_anim_cb(void *var, int32_t v)
{
    ...
    if (v >= 0) {
        lv_obj_set_y(xxx->plant, xxx->y * PLANT_Y_STEP + PLANT_Y_OFFSET + 9 + v / 2);
    } else {
        lv_obj_set_y(xxx->plant, xxx->y * PLANT_Y_STEP + PLANT_Y_OFFSET + 9 - v / 2);
    }
}
...
static void zb_create_cb(lv_timer_t *t)
{
    ...
    for (j = 0; j < max_zb_count; j++) {
        if (zb_matrix[j].alive == 0) {
            ...
            lv_img_set_zoom(zb_matrix[j].zb, 170);
            lv_img_set_src(zb_matrix[j].zb, zb_matrix[j].zb_class->zb_img_src);
            zb_matrix[j].y = rand() % 5;
            zb_matrix[j].x = ZB_SPAWN_X;
            lv_obj_set_pos(zb_matrix[j].zb, ZB_SPAWN_X, zb_matrix[j].y * ZB_Y_STEP + ZB_Y_OFFSET);
...
static void jumpzb_start_anim_cb(zb_type *xzb)
{
    ...
    lv_anim_set_values(&a, xzb->y * ZB_Y_STEP + ZB_Y_OFFSET, xzb->y * ZB_Y_STEP + ZB_Y_OFFSET - 20);
...
static void carzb_start_anim_cb(zb_type *xzb)
{
    ...
    lv_anim_set_values(&a, xzb->y * ZB_Y_STEP - 10, xzb->y * ZB_Y_STEP - 8);
...
static void qiqiuzb_start_anim_cb(zb_type *xzb)
{
    ...
    lv_anim_set_exec_cb(&a, jumpzb_jump_cb);
    lv_anim_set_time(&a, 1000);
    lv_anim_set_values(&a, xzb->y * ZB_Y_STEP - 10, xzb->y * ZB_Y_STEP + 5);
...
static void add_zidan_cb(lv_timer_t *t)
{
    ...
    for (i = 0; i < max_quantity; i++) {
        if (zidan[i].alive == 0) {
            zidan[i].alive = 1;
            zidan[i].x = x * PLANT_X_STEP + PLANT_X_OFFSET + 45;
            zidan[i].y = y;

            zidan[i].zidan = lv_btn_create(map1);
            lv_obj_set_pos(zidan[i].zidan, zidan[i].x, y * PLANT_Y_STEP + PLANT_Y_OFFSET + 11);
            lv_obj_set_size(zidan[i].zidan, 10, 10);
            ...
            lv_obj_set_style_radius(zidan[i].zidan, 5, LV_PART_MAIN);
            lv_obj_set_style_border_width(zidan[i].zidan, 1, LV_PART_MAIN);
...
static void timer_wogua_test_cb(lv_timer_t *t)
{
    ...
    if (wogua->blood > 0) {
        ...
        for (i = 0; i < max_zb_count; i++) {
            if (zb_matrix[i].blood > 0) {
                if ((zb_matrix[i].y == wogua->y) && (zb_matrix[i].x - wogua->x * PLANT_X_STEP < 53) &&
                    (zb_matrix[i].x - wogua->x * PLANT_X_STEP > 33)) {
                    wogua->blood = 0;
                    zb_matrix[i].blood = 0;
    ...
    if (dici->blood > 0) {
        ...
        for (i = 0; i < max_zb_count; i++) {
            if (zb_matrix[i].blood > 0 &&
                ...
                if ((zb_matrix[i].y == dici->y) && (zb_matrix[i].x - dici->x * PLANT_X_STEP < 33) &&
                    (zb_matrix[i].x - dici->x * PLANT_X_STEP > -33)) {
...
static void map_click_cb(lv_event_t *e)
{
    ...
    x = (click_point.x - GRID_OFFSET_X) / GRID_W;
    y = (click_point.y - GRID_OFFSET_Y) / GRID_H;
...
static void sunflower_creat_cb(card_btn_type *card_btn, int x, int y)
{
    ...
    for (j = 0; j < max_quantity; j++) {
        if (sunflower[j].alive == 0) {
            ...
            lv_img_set_zoom(sunflower[j].plant, 170);
            lv_img_set_src(sunflower[j].plant, &sun_img);
            lv_obj_set_pos(sunflower[j].plant, x * PLANT_X_STEP + PLANT_X_OFFSET + 9, y * PLANT_Y_STEP + PLANT_Y_OFFSET + 9);
...
static void wandou_creat_cb(card_btn_type *card_btn, int x, int y)
{
    ...
    for (j = 0; j < max_quantity; j++) {
        if (wandouflower[j].alive == 0) {
            ...
            lv_img_set_zoom(wandouflower[j].plant, 170);
            lv_img_set_src(wandouflower[j].plant, &wandou_img);
            lv_obj_set_pos(wandouflower[j].plant, x * PLANT_X_STEP + PLANT_X_OFFSET + 7, y * PLANT_Y_STEP + PLANT_Y_OFFSET + 9);
...
static void jiguangdou_creat_cb(card_btn_type *card_btn, int x, int y)
{
    ...
    for (j = 0; j < max_quantity; j++) {
        if (jiguangdou[j].alive == 0) {
            ...
            lv_img_set_zoom(jiguangdou[j].plant, 170);
            lv_img_set_src(jiguangdou[j].plant, &jiguangdou_img);
            lv_obj_set_pos(jiguangdou[j].plant, x * PLANT_X_STEP + PLANT_X_OFFSET + 7, y * PLANT_Y_STEP + PLANT_Y_OFFSET - 5);
...
static void add_jiguang_cb(lv_timer_t *t)
{
    ...
    jiguang_point[point_count][0].x = x * PLANT_X_STEP + PLANT_X_OFFSET + 45;
    jiguang_point[point_count][0].y = y * PLANT_Y_STEP + PLANT_Y_OFFSET + 29;
    jiguang_point[point_count][1].x = lv_disp_get_hor_res(lv_disp_get_default());
    jiguang_point[point_count][1].y = y * PLANT_Y_STEP + PLANT_Y_OFFSET + 29;
...
static void wogua_creat_cb(card_btn_type *card_btn, int x, int y)
{
    ...
    for (j = 0; j < max_quantity; j++) {
        if (wogua[j].alive == 0) {
            ...
            lv_img_set_zoom(wogua[j].plant, 170);
            lv_img_set_src(wogua[j].plant, &wogua_img);
            lv_obj_clear_flag(wogua[j].plant, LV_OBJ_FLAG_CLICKABLE);
            lv_obj_set_pos(wogua[j].plant, x * PLANT_X_STEP + PLANT_X_OFFSET + 7, y * PLANT_Y_STEP + PLANT_Y_OFFSET + 9);
...
static void dici_creat_cb(card_btn_type *card_btn, int x, int y)
{
    ...
    for (j = 0; j < max_quantity; j++) {
        if (dici[j].alive == 0) {
            ...
            lv_img_set_zoom(dici[j].plant, 170);
            lv_img_set_src(dici[j].plant, &dici_img);
            lv_obj_clear_flag(dici[j].plant, LV_OBJ_FLAG_CLICKABLE);
            lv_obj_set_pos(dici[j].plant, x * PLANT_X_STEP + PLANT_X_OFFSET + 7, y * PLANT_Y_STEP + PLANT_Y_OFFSET + 25);
...
static void lajiao_creat_cb(card_btn_type *card_btn, int x, int y)
{
    ...
    lv_img_set_zoom(lajiao, 170);
    lv_img_set_src(lajiao, &lajiao_img);
    lv_obj_set_pos(lajiao, x * PLANT_X_STEP + PLANT_X_OFFSET + 9, y * PLANT_Y_STEP + PLANT_Y_OFFSET + 9);
...
static void cherry_creat_cb(card_btn_type *card_btn, int x, int y)
{
    ...
    lv_img_set_zoom(cherry, 170);
    lv_img_set_src(cherry, &cherry_img);
    lv_obj_set_pos(cherry, x * PLANT_X_STEP + PLANT_X_OFFSET + 9, y * PLANT_Y_STEP + PLANT_Y_OFFSET + 9);
...
static void sunflower_creat_shine_cb(lv_timer_t *t)
{
    ...
    for (j = 0; j < max_quantity; j++) {
        if (shine[j].alive == 0) {
            ...
            lv_img_set_src(shine[j].shine, &sunshine);
            lv_obj_add_flag(shine[j].shine, LV_OBJ_FLAG_CLICKABLE);
            shine[j].x = x * PLANT_X_STEP + PLANT_X_OFFSET + 9;
            shine[j].y = y * PLANT_Y_STEP + PLANT_Y_OFFSET + 9;
            lv_obj_set_pos(shine[j].shine, shine[j].x, shine[j].y);
